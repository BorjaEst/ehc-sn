...
def test_grid_cell_toroid_x_cycling(layer_gen, scale):
    """Test that grid cell activation cycles correctly when moving along x-axis."""
    layer = layer_gen(spacing=scale)
    ...
    layer.set_position(reference_point)
    reference_activity = layer.activity.copy()

    layer.set_position(cycled_point)
...
@pytest.mark.parametrize("scale", [0.2])
def test_grid_cell_toroid_y_cycling(layer_gen, scale):
...
@pytest.mark.parametrize("scale", [0.2])
def test_grid_cell_toroid_diagonal_cycling(layer_gen, scale):
...
def test_grid_cell_multiple_cycles(layer_gen, distance_factor):
    """Test that grid cell activation repeats after multiple cycles."""
    layer = layer_gen(spacing=0.2)
...
def test_continuous_movement(layer_gen, position_gen, num_points):
    ...
    positions = position_gen(start, end, num_points)
    ...
    period_correlation = np.corrcoef(
        activities[0].flatten(), activities[expected_period_index].flatten()
    )[0, 1]
