...
class RunArguments(BaseSettings, extra="forbid", cli_parse_args=True):
    ...
    project_name: Optional[str] = Field(
        default=None,
        description=(
            "Project name. If not set, it defaults to the capitalized name of the dataset " "(e.g. `Arena` -> `Arena EHC-torch`)."
        ),
    )
    run_name: Optional[str] = Field(
        default=None,
        description=("Run name. If not set, it defaults to `<arch_name> <random_slug>` " "(e.g. `EhcV1 arena-baseline cool-slug`)."),
    )
    ...
    objective: EHCObjectiveConfig = Field(
        ...,
        description="EHC objective configuration (observation, latent, regularization).",
    )
    ...
    runtime: RuntimeConfig = Field(
        ...
        description="EHC runtime dynamics schedule settings applied inside the training loop.",
    )
    ...
    def ehc_config(self) -> ModelConfig_EHC_V1:
        """Compose ModelConfig_EHC_V1 from leaf settings."""
        return ModelConfig_EHC_V1.model_validate(self, from_attributes=True)
...
if __name__ == "__main__":
    ...
    # Start training.
    # - The LightningModule wraps the EHC model and defines the training loop.
    # - The DataModule constructs loaders for the arena replay dataset.
    trainer.fit(
        # Lightning module: training step, optimizer and schedule setup.
        model=TrainingModel(settings.ehc_config),
