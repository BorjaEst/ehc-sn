"""
TEM v1 baseline training script.
This script trains the TEM v1 model on the arena replay dataset using PyTorch
Lightning. It is configured via a TOML file specified by the
TEM_V1_CONFIGURATION_PATH environment variable. The configuration includes
model architecture, data settings, training control, optimizer and scheduler
settings, and logging/checkpointing options.
"""
...
class RunArguments(BaseSettings, cli_parse_args=True, cli_kebab_case=True):
    """Settings for TEM v1 baseline training run."""
    ...
    @property
    def tem_config(self) -> TEMV1ModelConfig:
        """Compose TEMV1ModelConfig from leaf settings."""
        return TEMV1ModelConfig.model_validate(self, from_attributes=True)
...
if __name__ == "__main__":
    ...
    training_model = TEMV1TrainingModel(settings.tem_config)
