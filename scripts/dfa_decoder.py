"""
DFA Decoder for Entorhinal-Hippocampal Circuit

Simple script for training only the decoder part of a pretrained autoencoder
using DFA (Direct Random Target Projection). Uses TOML configuration files
for parameter management.

Usage:
    python dfa_decoder.py
    python dfa_decoder.py --config experiments/decoder.toml
    python dfa_decoder.py --pretrained models/baseline.ckpt --epochs 100
"""
...
from ehc_sn.models.decoders import DFALinear as DFADecoder
...
class DFADecoderTrainingSettings(BaseModel):
    """
    Configuration settings for DFA decoder-only training experiment.
    ...
    experiment_name: str = Field(default="dfa_decoder", description="Experiment name")
...
# -------------------------------------------------------------------------------------------
# DFA Decoder Training Pipeline
# -------------------------------------------------------------------------------------------


class DFADecoderTrainingPipeline:
    """Orchestrates the DFA decoder-only training workflow."""

    def __init__(self, settings: DFADecoderTrainingSettings):
        """
        Initialize DFA decoder training pipeline with settings.
    ...
    def setup_components(self) -> None:
        ...
        print("🔧 Setting up DFA decoder training components...")
        ...
        print("🔄 Replacing decoder with DFADecoder")
        # Create DFA decoder parameters with Tanh activation
        dfa_decoder_params = DecoderParams(
            output_shape=self.settings.input_shape,
            latent_dim=self.settings.latent_dim,
            activation_fn=nn.Tanh,  # Use Tanh for DFA
        )
    ...
    def display_configuration(self) -> None:
        ...
        print("\n📋 DFA DECODER TRAINING CONFIGURATION")
        print("-" * 50)
    ...
    def train_decoder(self) -> None:
        ...
        print("🎉 DFA decoder training completed successfully!")

    # -----------------------------------------------------------------------------------

    def evaluate_and_visualize(self) -> None:
        """Evaluate DFA decoder performance and generate visualizations."""
        if not self.components:
            raise RuntimeError("Components not initialized. Call setup_components() first.")

        print("\n📊 Evaluating DFA decoder performance...")
    ...
    def run_complete_experiment(self) -> None:
        """Execute the complete DFA decoder training pipeline."""
...
def run_dfa_decoder_experiment(
    ...
    """
    Run the complete DFA decoder training experiment.
    ...
    """
    print("=" * 80)
    print("🧠 ENTORHINAL-HIPPOCAMPAL CIRCUIT: DFA DECODER TRAINING")
    print("=" * 80)
    ...
    settings = DFADecoderTrainingSettings(**settings_dict)

    # Run experiment pipeline
    pipeline = DFADecoderTrainingPipeline(settings)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="DFA Decoder training for Entorhinal-Hippocampal Circuit",
