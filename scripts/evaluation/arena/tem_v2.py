#!/usr/bin/env python3
"""Evaluate TEM-V2 on arena — convenience alias around ``run_eval.py run``."""
...
DEFAULT_CONFIG = Path("config/evaluation/tem-v2-arena.toml")
DEFAULT_GALLERY_DIR = Path("artifacts/inspection/tem-v2-arena")
DEFAULT_EVAL_ARTIFACT_DIR = Path("artifacts/eval/tem-v2-arena")

app = typer.Typer(no_args_is_help=True, help="Evaluate TEM-V2 on arena.")
...
def run(
    ...
    """Execute a checkpoint against the arena TEM-V2 experiment."""
    try:
        loaded = load_evaluation_experiment(
            config,
            expected_experiment_id="tem-v2-arena",
        )
