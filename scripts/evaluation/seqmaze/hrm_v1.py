#!/usr/bin/env python3
"""Evaluate HRM-V1 on seqmaze — convenience alias around ``run_eval.py run``."""
...
DEFAULT_CONFIG = Path("config/evaluation/hrm-v1-seqmaze.toml")
DEFAULT_GALLERY_DIR = Path("artifacts/inspection/hrm-v1-seqmaze")
DEFAULT_EVAL_ARTIFACT_DIR = Path("artifacts/eval/hrm-v1-seqmaze")

app = typer.Typer(no_args_is_help=True, help="Evaluate HRM-V1 on seqmaze.")
...
def run(
    ...
    """Execute a checkpoint against the seqmaze HRM-V1 experiment."""
    try:
        loaded = load_evaluation_experiment(
            config,
            expected_experiment_id="hrm-v1-seqmaze",
        )
