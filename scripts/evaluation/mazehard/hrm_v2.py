#!/usr/bin/env python3
"""Evaluate HRM-V2 on mazehard — convenience alias around ``run_eval.py run``."""
...
DEFAULT_CONFIG = Path("config/evaluation/hrm-v2-mazehard.toml")
DEFAULT_GALLERY_DIR = Path("artifacts/inspection/hrm-v2-mazehard")
DEFAULT_EVAL_ARTIFACT_DIR = Path("artifacts/eval/hrm-v2-mazehard")

app = typer.Typer(no_args_is_help=True, help="Evaluate HRM-V2 on mazehard.")
...
def run(
    ...
    """Execute a checkpoint against the mazehard HRM-V2 experiment."""
    try:
        loaded = load_evaluation_experiment(
            config,
            expected_experiment_id="hrm-v2-mazehard",
        )
