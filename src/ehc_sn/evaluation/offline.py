"""Offline evaluation runner — checkpoint + config → evaluation artifact.

Produces a persisted evaluation-artifact bundle from a model-family checkpoint
without a Lightning Trainer or training callback.  Uses only public APIs
from ``ehp_sn.evaluation``.
...
Boundary rules:
    - Must not import from ``ehp_sn.reporting`` or ``ehp_sn.report_metrics``.
    - Must not call private ``_build_*`` or ``_hydrate_*`` from
      ``ehp_sn.evaluation.artifacts``.
...
from ehc_sn.evaluation.executor import iter_evaluation_regime
from ehc_sn.evaluation.invocation import (
    EvaluationExecutionRequest,
    EvaluationPrecision,
)
from ehc_sn.evaluation.reuse import (
