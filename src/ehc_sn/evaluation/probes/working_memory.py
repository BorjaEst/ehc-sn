...
be persisted as probe artifacts (``.npz`` + ``.json``) alongside evaluation artifacts,
    ...
    for t in range(T):
        ...
        for prefix, X in [("h_", X_h), ("l_", X_l)]:
            ...
            for train_idx, test_idx in cv.split(X, y):
                ...
                ap_folds.append(average_precision_score(y[test_idx], y_prob))
...
def compute_pfc_path_memory_probe_from_artifact(
    ...
    *,
    ...
    """Load an evaluation artifact and compute the path-memory probe.
...
def compute_and_persist_probes_for_artifact(
    ...
    *,
    ...
    """Compute the PFC path-memory probe for an evaluation artifact and persist it.
    ...
    ----------
    artifact_path:
        Path to an evaluation artifact directory (containing ``_SUCCESS`` and
        ``manifest.json``).
    output_dir:
    ...
    ------
    FileNotFoundError
        If the evaluation artifact does not exist or lacks required traces.
    """
    from ehc_sn.evaluation.artifacts import load_artifact_run_cases
    ...
    if not cases:
        raise FileNotFoundError(
            f"No cases found in evaluation artifact: {artifact_path}"
        )
