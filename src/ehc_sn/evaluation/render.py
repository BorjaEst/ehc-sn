...
:class:`~ehp_sn.callbacks.evaluation.EvaluationRegimesCallback` so that the
callback owns only policy (scheduling, failure handling, logging) and the
rendering logic is a pure function in the ``evaluation`` package.

Usage::

    from ehc_sn.evaluation.render import (
        RenderedPreview,
    ...
    previews = render_regime_preview_figures(
        ...
        figure_names=["prediction_reasoning_mazehard"],
