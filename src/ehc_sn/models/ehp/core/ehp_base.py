"""Shared settings and constants for EHP backbone versions.

This module intentionally stays thin. It holds only:
  - Shared projection edge settings grouped for EHP v2 (and reusable by later versions).
...
# ---------------------------------------------------------------------------
# Fixed body-workspace slot names shared across EHP versions.
# ---------------------------------------------------------------------------
...
class EHCProjectionSettings(BaseModel, extra="forbid", strict=False):
    """Shared inter-region projection settings for EHP backbones.
    ...
    pfc_to_hpc: ProjectionSettings = Field(
        default_factory=lambda: ProjectionSettings(
            mode="linear", bridge="broadcast", init="random", learnable=True
        ),
    ...
    hpc_to_pfc: ProjectionSettings = Field(
        default_factory=lambda: ProjectionSettings(
            mode="linear", init="random", learnable=True
        ),
        description=(
