    ...
    def trace_views(  # -------------------------------------------------------
        ...
        if "hpc.memory.summary" in requested:
            ...
            result["hpc.memory.summary"] = {
                ...
                "write_count": (int(memory.g_cued.to_dense().shape[-1])),
                "occupancy_ratio": 1.0,
            }
        if "hpc.memory.write_event" in requested:
            # Reserved for future sparse write capture; returns None
            # from the per-step path that does not track write events.
