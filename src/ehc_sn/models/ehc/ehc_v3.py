"""EHC v3 backbone model, settings, state, and forward I/O.

EHC v3 (Entorhinal-Hippocampal Circuit, version 3) extends EHC v2 with a
Zheng-style current-step cortical cue path.  It is otherwise identical to v2.

Cue-timing contract (V3 — explicit difference from V2):
    V2 derives the cortical cue from *previous*-step ``state.pfc.summary``.
    V3 derives the cortical cue from the *current*-step PFC output via an
...
# =============================================================================
@dataclass
class EHCOutputV3(DetachMixin):
    """Task-agnostic output payload for one EHC v3 forward step.
    ...
    control: EHCControlV3
    content: EHCContentV3
...
# =============================================================================
@dataclass
class EHCInputV3(DetachMixin):
    """Task-agnostic input payload for EHC v3 forward steps.

    Attributes:
        sensory_codes:   Multi-scale sensory observation codes.
...
class ModelSettingsV3(BaseModel, extra="forbid", strict=False):
    ...
    def _validate_pfc_seq_length(self) -> "ModelSettingsV3":
        if self.pfc.seq_length < 4:
            raise ValueError(f"pfc.seq_length must be >= 4 (3 fixed body slots + at least 1 content slot), " f"got {self.pfc.seq_length}.")
        return self
...
class EHCModelV3(nn.Module):
    """EHC v3 backbone: TEM circuit with PFC reasoning and STR control.
    ...
    def __init__(  # ------------------------------------------------------------------
        self,
        config: ModelSettingsV3,
        *,
        device: Optional[Device] = None,
        dtype: Optional[Dtype] = None,
    ) -> None:
        """Construct EHC v3 from resolved model settings."""
        super().__init__()
        self._config = config
    ...
    @property
    def config(self) -> ModelSettingsV3:
    ...
    def init_state(  # -----------------------------------------------------------
        ...
        *,
        ...
        """Create an initial full-batch recurrent EHC v3 state.
        ...
        Returns:
            Freshly initialized :class:`EHCStateV3`.
        """
    ...
    def reset_state(  # ----------------------------------------------------------
        ...
        Returns:
            Updated :class:`EHCStateV3` with flagged rows reset to initial values.
        """
    ...
    def forward(  # --------------------------------------------------------------
        self,
        inputs: EHCInputV3,
        state: Optional[EHCStateV3] = None,
    ) -> tuple[EHCOutputV3, EHCStateV3]:
        """Run one EHC v3 step and return architecture-native latents.

        Cue-timing (V3 contract — explicit delta from V2):
            V2 derives ``c_prop`` from *previous*-step ``state.pfc.summary``.
            V3 derives ``c_prop`` from a first PFC pass on the current step,
        ...
        Args:
            inputs: Task-agnostic EHC v3 input payload.
        ...
        # --- V3 uses current-step PFC output as the cue source (not previous-step) ---
        ...
        # (In V2 this is step 8; in V3 it precedes the provisional PFC pass so
        ...
        # =====================================================================
        # Stage 1 — Cue-generation PFC pass (V3 explicit delta).
        ...
        # --- V3 cue: current-step PFC summary (first pass) -------------------
        # c_prop: project current-step (first-pass) PFC summary into the
        #         hippocampal multi-frequency cue family via pfc_to_hpc.
        # c_mem:  reinstated contextual evidence from bank c — deferred in V3.
        # c_use:  routed cue for replay bias and bank-c writes; equals c_prop in V3.
        ...
        c_use: list[Tensor] = c_prop  # V3: use current-step cortical cue proposal
        ...
        # EHCContentV3 exposes post-reasoning PFC workspace slots (second pass).
        content = EHCContentV3(
        ...
        return EHCOutputV3(control=control, content=content, grid_codes=grid_codes, place_codes=place_codes), state
