...
class ProjectionSettings(BaseModel, extra="forbid"):
    ...
    bridge: ProjectionBridge = Field(
        default="auto",
        description="Optional bridge override used to reconcile mismatched endpoint structures.",
    )
    init: InitStrategy = Field(
        default="identity",
        description="Initialization strategy: 'identity' for structured identity-like initialization or 'random'.",
    )
    learnable: bool = Field(
        default=False,
        description="If True, projection parameters remain trainable after construction and reset.",
    )
...
ProjectionModuleEdge: TypeAlias = tuple[ProjectionEndpointValue, ProjectionEndpointValue, ProjectionSettings]
ProjectionNamedEdge: TypeAlias = tuple[str, str, ProjectionSettings]


def flat_endpoint(width: int) -> FlatEndpointSpec:
    """Return a typed flat endpoint descriptor."""
    return FlatEndpointSpec(width=int(width))


def multiscale_endpoint(component_or_shape: TEMComponent | Sequence[int]) -> MultiScaleEndpointSpec:
...
class ProjectionModule(nn.Module):
    ...
    def __init__(self, z_from: TEMComponent | Sequence[int], z_to: TEMComponent | Sequence[int], settings: ProjectionSettings):
        super().__init__()
        self._shape_from = _coerce_shape(z_from)
        self._shape_to = _coerce_shape(z_to)
        if len(self._shape_from) != len(self._shape_to):
            raise ValueError("Aligned ProjectionModule requires the same number of bands on source and target.")
    ...
    @staticmethod
    def create_module(settings: ProjectionSettings, shape_from: list[int], shape_to: list[int]) -> "AbstractProjectionModule":
...
class PerBandLinear(nn.Module):
        ...
        *,
        ...
        if isinstance(shape_to, int):
            ...
            if len(self._shape_to) != len(self._shape_from):
                raise ValueError(f"shape_to must have length {len(self._shape_from)}, got {len(self._shape_to)}.")

        self.layers = nn.ModuleList(
            [
                nn.Linear(in_features, out_features, bias=bias, device=device, dtype=dtype)
                for in_features, out_features in zip(self._shape_from, self._shape_to, strict=True)
            ]
        )
    ...
    def forward(self, codes: Sequence[Tensor], *, dtype: Optional[torch.dtype] = None) -> list[Tensor]:
        """Project one tensor per band after validating aligned widths."""
        if len(codes) != len(self.layers):
            raise ValueError(f"codes must have length {len(self.layers)}, got {len(codes)}.")

        outputs: list[Tensor] = []
        for index, (code, layer, width) in enumerate(zip(codes, self.layers, self._shape_from, strict=True)):
            if code.ndim != 2 or int(code.shape[1]) != width:
                raise ValueError(f"codes[{index}] must have shape (B, {width}), got {tuple(code.shape)}.")
...
class ProjectionBundle(nn.Module):
    ...
    def __init__(self, *, endpoints: Mapping[str, ProjectionEndpointSpec], edges: Mapping[str, ProjectionEdgeSpec]) -> None:
        super().__init__()
        ...
        for name, edge in self._edge_specs.items():
            if edge.source not in self._endpoints:
                raise KeyError(f"Unknown projection source endpoint: {edge.source!r}.")
            if edge.target not in self._endpoints:
                raise KeyError(f"Unknown projection target endpoint: {edge.target!r}.")
    ...
    def from_named_modules(
        ...
        *,
        ...
        """
        endpoint_specs = {name: _coerce_endpoint_spec(value) for name, value in modules.items()}
        edge_specs = {
            name: ProjectionEdgeSpec(source=source, target=target, settings=settings) for name, (source, target, settings) in edges.items()
        }
...
class IdentityModule(AbstractProjectionModule):
    ...
    def __init__(self, shape_from: list[int], shape_to: list[int]) -> None:
        super().__init__()
        if shape_from != shape_to:
            raise ValueError("identity projections require matching source and target shapes.")
...
class _BaseMatrixProjectionModule(AbstractProjectionModule):
    ...
    def __init__(self, matrices: list[Tensor], *, learnable: bool) -> None:
        super().__init__()
        self.w = nn.ParameterList([nn.Parameter(matrix, requires_grad=learnable) for matrix in matrices])

    def forward(self, z_from: Sequence[Tensor]) -> list[Tensor]:
        return [torch.matmul(z_from[f], self.w[f]) for f in range(len(z_from))]

    def inverse(self, z_to: Sequence[Tensor]) -> list[Tensor]:
        return [torch.matmul(z_to[f], self.w[f].T) for f in range(len(z_to))]

    def _copy_matrices(self, matrices: list[Tensor]) -> None:
        if len(matrices) != len(self.w):
            raise ValueError(f"Expected {len(self.w)} matrices, got {len(matrices)}.")
...
class LinearModule(_BaseMatrixProjectionModule):
    ...
    def __init__(self, shape_from: list[int], shape_to: list[int], settings: ProjectionSettings) -> None:
        self._shape_from = list(shape_from)
        self._shape_to = list(shape_to)
        self._settings = settings
        super().__init__(self.create_matrices(shape_from, shape_to, settings), learnable=settings.learnable)
    ...
    def create_matrices(shape_from: list[int], shape_to: list[int], settings: ProjectionSettings) -> list[Tensor]:
        if settings.init == "identity":
            return [_create_linear_identity_matrix(n_in, n_out) for n_in, n_out in zip(shape_from, shape_to, strict=True)]
    ...
    def reset_parameters(self) -> None:
        self._copy_matrices(self.create_matrices(self._shape_from, self._shape_to, self._settings))
...
class LowRankModule(AbstractProjectionModule):
    ...
    def reset_parameters(self) -> None:
        w_down, w_repeat = self.create_matrices(self._shape_from, self._shape_to, self._settings)
...
class _FlatProjectionEdge(nn.Module):
    ...
    def __init__(self, width_from: int, width_to: int, settings: ProjectionSettings) -> None:
        super().__init__()
        self._width_from = int(width_from)
        self._width_to = int(width_to)
        self.projection = ProjectionModule([self._width_from], [self._width_to], settings)
...
class _TokenSequenceProjectionEdge(nn.Module):
    ...
    def __init__(self, width_from: int, width_to: int, settings: ProjectionSettings) -> None:
        ...
        self._width_from = int(width_from)
        self._width_to = int(width_to)
        self.projection = _FlatProjectionEdge(self._width_from, self._width_to, settings)
    ...
    def inverse(self, y: Tensor) -> Tensor:
        ...
        x = self.projection.inverse(y.reshape(batch_size * seq_len, self._width_to))
...
class _BroadcastProjectionEdge(nn.Module):
    ...
    def __init__(self, width_from: int, shape_to: Sequence[int], settings: ProjectionSettings) -> None:
        super().__init__()
        self._width_from = int(width_from)
        self._shape_to = [int(width) for width in shape_to]
        self._n_bands = len(self._shape_to)
        self.projection = ProjectionModule([self._width_from] * self._n_bands, self._shape_to, settings)
    ...
    def inverse(self, y: Sequence[Tensor]) -> Tensor:
        raise NotImplementedError("broadcast flat->multiscale projections do not define a canonical inverse.")
    ...
    *,
    ...
    if bridge == "broadcast":
        if source.kind == "flat" and target.kind == "multiscale":
            return _BroadcastProjectionEdge(source.width, target.shape, settings)
        raise ValueError(f"broadcast bridge supports flat -> multiscale only, got {source.kind!r} -> {target.kind!r}.")

    raise ValueError(f"Unknown projection bridge: {bridge}")


# =================================================================================================
def _coerce_rank_list(rank: int | Sequence[int] | None, shape_from: Sequence[int], shape_to: Sequence[int]) -> list[int]:
    """Normalize low-rank config into one per-band rank list and validate it."""
    n_freq = len(shape_from)
    if rank is None:
        rank_list = [math.gcd(int(n_in), int(n_out)) for n_in, n_out in zip(shape_from, shape_to, strict=True)]
    ...
    for f, (n_in, n_out, r) in enumerate(zip(shape_from, shape_to, rank_list, strict=True)):
        if r <= 0:
...
def _coerce_shape(component_or_shape: TEMComponent | Sequence[int]) -> list[int]:
    """Return a validated multiscale shape from either a component or a raw shape list."""
    if isinstance(component_or_shape, Sequence) and all(isinstance(width, int) for width in component_or_shape):
    ...
    if any(width <= 0 for width in shape):
        raise ValueError(f"projection shapes must contain positive widths, got {shape}.")
    return shape
...
def _resolve_bridge(
    ...
    *,
    ...
    if source.kind == "flat" and target.kind == "multiscale":
        return "broadcast"
    raise ValueError(f"Could not resolve projection bridge automatically for {source.kind!r} -> {target.kind!r}.")


def _create_linear_identity_matrix(n_in: int, n_out: int) -> Tensor:
...
def _validate_flat_tensor(tensor: Tensor, width: int, *, name: str) -> None:
...
def _validate_token_sequence_tensor(tensor: Tensor, width: int, *, name: str) -> None:
    """Validate one token-sequence tensor with shape ``(B, S, width)``."""
    if tensor.ndim != 3 or int(tensor.shape[2]) != width:
        raise ValueError(f"{name} must have shape (B, S, {width}), got {tuple(tensor.shape)}.")
