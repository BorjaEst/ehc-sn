...
from ehc_sn.modules.transformer import (
    ...
    def __init__(  # ----------------------------------------------------------
        ...
        *,
        ...
        if summary_config.hidden_size != hidden_size:
            raise ValueError(
                "summary_config.hidden_size must match hidden_size, got "
                f"{summary_config.hidden_size} and {hidden_size}."
            )
        ...
        self.summary = TransformerSequenceSummary(
            summary_config, device=device, dtype=dtype
        )
        ...
        *,
        ...
        if self.token_embedding.padding_idx is not None:
            self.token_embedding.weight.data[
                self.token_embedding.padding_idx
            ].zero_()
        ...
        *,
        ...
        if attn_mask is not None:
            if attn_mask.shape != input_ids.shape:
                raise ValueError(
                    f"attn_mask must have shape {tuple(input_ids.shape)}, got {tuple(attn_mask.shape)}."
                )
        ...
        if min_id < 0 or max_id >= self.token_embedding.num_embeddings:
            raise ValueError(
                "input_ids must lie within "
                f"[0, {self.token_embedding.num_embeddings - 1}], got min={min_id}, max={max_id}."
            )
        token_embeddings = self.token_embedding(input_ids.to(torch.int32))
        return self.summary(
            self._embedding_scale * token_embeddings, attn_mask=attn_mask
        )
