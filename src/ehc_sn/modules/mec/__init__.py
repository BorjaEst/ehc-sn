...
class MECSettings(BaseModel, extra="forbid"):
    ...
    do_sample: bool = Field(
        default=False,
        description=(
            "Whether to sample from distributions (stochastic) or use means "
            "(deterministic). Sampling policy is centralized in MECModel."
        ),
    )
    ...
    def mec_layout(self) -> MECLayout:
        ...
        return resolve_mec_layout(
            self.grid_shape, ovc_mode=self.ovc.mode, ovc_shape=self.ovc.shape
        )
...
class MECState(DetachMixin):
    ...
    abstract_belief: (
        LocationBelief  # State and uncertainty over abstract locations
    )
    _n_ovc_modules: Optional[int] = (
        None  # Cached number of appended OVC modules
    )
    ...
    def new(  # ---------------------------------------------------------------
        ...
        return replace(
            self,
            abstract_belief=LocationBelief(mean=cells, uncertainty=uncertainty),
        )
...
class MECModel(nn.Module):
    ...
    def __init__(  # ----------------------------------------------------------
        ...
        *,
        ...
        init_fn = lambda size: truncnorm.rvs(
            -2, 2, size=size, loc=0, scale=config.sigma_init
        )
        self.cells_init = nn.ParameterList(
            [
                nn.Parameter(torch.tensor(init_fn(n), dtype=torch.float32))
        ...
        self.path_integration = PathIntegrator(
            action_count, self._shape, f_initial, config=config.path
        )
        self.p2g_correction = P2GMemory(
            n_hippocampal, self._shape, config=config.p2g
        )
    ...
    def init_state(  # --------------------------------------------------------
        ...
        """
        g0 = [
            g.unsqueeze(0).expand(batch_size, -1).to(device)
            for g in self.cells_init
        ]
        sigma_0 = [
            std.unsqueeze(0).expand(batch_size, -1).to(device)
            for std in self.uncertainty_init
        ]
    ...
    def forward(  # -----------------------------------------------------------
        ...
        *,
        ...
        raise NotImplementedError(
            "MEC forward not implemented. Use generative() or inference()."
        )
    ...
    def generative(  # --------------------------------------------------------
        ...
        if landmark_id is not None:
            no_direct_mask = landmark_id.squeeze(-1).to(torch.int64) != 0
            if not torch.any(no_direct_mask):
                no_direct_mask = None

        # 1) Action-driven transition for the state (legacy g_path)
        transition = self.path_integration(
            action, state.cells, no_direct_mask=None
        )
        ...
        if no_direct_mask is not None:
            g_gen = self._clamp(
                self.path_integration.mean(action, state.cells, no_direct_mask)
            )
        ...
        if torch.any(reset_mask):
            g_gen, next_state = self._preserve_reset_rows(
                ...
                g_gen,
                state,
                state.new(cells_next, transition.uncertainty),
            )
    ...
    def inference(  # ---------------------------------------------------------
        ...
        state: MECState,
        *,
        correction_error: list[Tensor] | None = None,
    ) -> tuple[AbstractLocation, MECState]:
        ...
        transition = (
            self.p2g_correction(p_x, transition, quality_error=correction_error)
            if p_x is not None
            else transition
        )
        # Step 2: Apply OVC correction from shiny landmarks.
        transition = (
            self.ovc_correction(landmark_id, transition)
            if self.config.ovc.mode != "off"
            else transition
        )
    ...
    def _clamp(  # ------------------------------------------------------------
        ...
        return [
            torch.clamp(
                g_f, min=self._config.clamp_min, max=self._config.clamp_max
            )
            for g_f in g
        ]
    ...
    def _normalize_reset_mask(  # ---------------------------------------------
        ...
        *,
        ...
        return (
            episode_start.reshape(batch_size, -1)
            .any(dim=1)
