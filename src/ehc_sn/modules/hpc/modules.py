...
from ehc_sn.modules.hpc.query import (
    AttractorRead,
...
class HPCAttractor(HPCBase):
    ...
    def __init__(  # ----------------------------------------------------------
        ...
        *,
        ...
        masks = utils.update_to_masks(
            self.shape,
            update=utils.make_update_hierarchical(n_stages, self.n_freq),
        )
        self.register_buffer("masks_hierarchical", masks, persistent=False)
        masks = utils.update_to_masks(
            self.shape, update=utils.make_update_full(n_stages, self.n_freq)
        )
        ...
        self.write_module = HebbianWrite(
            config.write, device=device, dtype=dtype
        )
        self.store_backend = DenseHebbianStoreBackend(
            self.write_module, layout=hebbian_layout
        )
    ...
    def _recall_flat_impl(  # -------------------------------------------------
        ...
        *,
        ...
        masks = (
            self.masks_hierarchical if role == "generative" else self.masks_full
        )
        return self.retrieval_module(
            query, memory.as_linear_view(), masks=masks
        )
    ...
    def _update_memory_impl(  # -----------------------------------------------
        ...
        if named_writes:
            ...
            raise TypeError(
                f"{type(self).__name__} does not support named bank writes: {names}."
            )
        g_cued = self.store_backend.apply_write(
            memory.g_cued, key, g_value, masked=True
        )
        x_cued = g_cued if self.config.common_memory else memory.x_cued
        if not self.config.common_memory and x_value is not None:
            x_cued = self.store_backend.apply_write(
                memory.x_cued, key, x_value, masked=False
            )
        return MemoryState(g_cued=g_cued, x_cued=x_cued)
...
class HPCAttention(HPCBase):
    ...
    def _init_memory_impl(  # -------------------------------------------------
        ...
        *,
        ...
        x_cued = (
            g_cued
            if self.config.common_memory
            else self.store_backend.init_store(batch_size, device=device)
        )
        return MemoryState(g_cued=g_cued, x_cued=x_cued)
    ...
    def _update_memory_impl(  # -----------------------------------------------
        ...
        if self.config.common_memory:
            ...
            x_cued = self.store_backend.append_write(
                memory.x_cued, key, x_value
            )

        if named_writes:
            available_bank_names = set(memory.g_cued.bank_names) | set(
                memory.x_cued.bank_names
            )
            missing_bank_names = sorted(
                set(named_writes) - available_bank_names
            )
            if missing_bank_names:
                names = ", ".join(repr(name) for name in missing_bank_names)
                raise ValueError(
                    f"Named factor-memory banks {names} are not available for "
                    "writing."
                )
    ...
    def recall(  # ------------------------------------------------------------
        ...
        *,
        ...
        recalled = self.retrieval_module.recall_from_evidence(
            evidence, memory.as_factor_view()
        )
