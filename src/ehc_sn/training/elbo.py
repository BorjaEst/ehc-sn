...
def gaussian_kl_divergence(  # ------------------------------------------------
    ...
    *,
    ...
    kl = (
        log_std_ratio
        + (posterior_var + mean_delta_sq) / (2.0 * prior_var)
        - 0.5
    )
    return kl.sum(dim=-1)
...
def sum_gaussian_kl_divergence(  # --------------------------------------------
    posterior_mean: LatentCode,
    posterior_std: LatentCode,
    prior_mean: LatentCode,
    ...
    *,
    ...
    if not (
        n_blocks
        == len(posterior_std_blocks)
        == len(prior_mean_blocks)
        == len(prior_std_blocks)
    ):
        raise ValueError(
            "Posterior and prior moments must expose the same number of latent blocks."
        )
    ...
    for post_mean, post_std, prev_mean, prev_std in zip(
        ...
        term = gaussian_kl_divergence(
            post_mean, post_std, prev_mean, prev_std, min_std=min_std
        )
