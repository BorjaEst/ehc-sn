"""EHC-specific weight-loading helpers.

Hydrate named semantic-group weights from checkpoints into the EHC model.
"""
...
# Mapping from public semantic group name to EHC model state-dict prefixes.
_GROUP_TO_PREFIXES: dict[str, tuple[str, ...]] = {
    "spatial_core": ("hpc", "mec", "lec", "projections"),
    "controller_bridge": ("pfc",),
    "controller_heads": ("str",),
    "all": (),
}

VALID_INIT_GROUPS: frozenset[str] = frozenset(_GROUP_TO_PREFIXES)
"""Set of valid semantic group names for EHC init-only hydration."""
...
def load_weights_from_checkpoint(  # ------------------------------------------
    ...
    """Hydrate named EHC semantic-group weights from a checkpoint into *model*.
    ...
    Args:
        model: Target EHC model to hydrate in place.
