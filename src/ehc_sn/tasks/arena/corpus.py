"""Arena corpus reader — load persisted arrays from a versioned root.

Pattern mirrors ``routebind/corpus.py``.
"""
...
from ehc_sn.tasks.arena.builder import ARENA_TASK_CHANNELS

_ARENA_CHANNELS: tuple[str, ...] = tuple(ARENA_TASK_CHANNELS)
...
def load_split_arrays(root: Path, split: str) -> dict[str, np.ndarray] | None:
    ...
    Args:
        root: Versioned corpus root
            (e.g. ``data/processed/arena/default/v1``).
...
def load_sample(
    ...
    split: str,
    index: int,
    channels: tuple[str, ...] = _ARENA_CHANNELS,
) -> dict[str, np.ndarray]:
    ...
    Returns:
        Dict mapping channel names to arrays for one sample.
