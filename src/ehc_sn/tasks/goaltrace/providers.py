"""Task-owned evaluation case providers for the Goaltrace task family.

These providers implement :class:`~ehc_sn.eval.contracts.EvaluationSourceProvider`
and supply batched Goaltrace replay cases to named evaluation regimes.

Ownership rules:
    - Providers live here, next to task evaluation code.
    - They know Goaltrace channel semantics and index conventions.
...
from ehc_sn.tasks.goaltrace.traces import GoaltraceEvaluationSourceContext


# =============================================================================
class GoaltraceReplayProvider:
    """Goaltrace-task-owned provider for processed replay evaluation.

    Loads from a versioned processed Goaltrace task corpus and yields batched
    cases for named evaluation regimes.

    ``provider_settings`` keys:

    - ``dataset_path`` (*str*, required): Path to the processed Goaltrace task
        ...
        *,
        ...
        if not (self._dataset_path / "index.jsonl").exists():
            raise FileNotFoundError(
                ...
                f"Goaltrace corpus root (e.g. data/processed/goaltrace/default/v1)."
            )
    ...
    def provide_cases(
        ...
        """Yield batched Goaltrace replay cases from the configured split.
        ...
        if not entries:
            raise ValueError(
                f"No Goaltrace entries found for split={self._split!r} "
                f"at {data_root}."
            )
...
__all__ = ["GoaltraceReplayProvider"]
