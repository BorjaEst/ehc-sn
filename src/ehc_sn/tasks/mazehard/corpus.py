"""MazeHard corpus reader — load persisted arrays from a versioned root.
...
_MAZEHARD_CHANNELS: tuple[str, ...] = tuple(MAZEHARD_TASK_CHANNELS)
...
def load_split_arrays(root: Path, split: str) -> dict[str, np.ndarray] | None:
    ...
    Args:
        root: Versioned corpus root
            (e.g. ``data/processed/mazehard/default/v1``).
...
def load_sample(
    ...
    split: str,
    index: int,
    channels: tuple[str, ...] = _MAZEHARD_CHANNELS,
) -> dict[str, np.ndarray]:
    ...
    Args:
        ...
        channels: Channel names to load (default: all mazehard channels).
