"""TorchRL EnvBase for routebind field-prediction (stub).

Routebind is a single-step field prediction task with no interactive
...
class EnvConfig(BaseModel, extra="forbid"):
    """Configuration for :class:`RoutebindEnv`.

    Attributes:
        max_episode_steps: Maximum steps per episode before truncation.
        schema_length: Total schema-token sequence length (S = 900).
        halt_action: Action index interpreted as 'halt'.
    """

    max_episode_steps: int = Field(
        default=8,  # deliberation steps, not physical moves
        ge=1,
    ...
    schema_length: int = Field(
        ...,
        ge=1,
        description="Schema-token sequence length (S = num_spatial_slots).",
    )
...
# =============================================================================
class RoutebindEnv(EnvBase):
    """Minimal TorchRL environment for routebind field-prediction.
    ...
    does not implement state transitions — the observed task (cell types,
    observation IDs, flags) is static across deliberation steps.
    """
