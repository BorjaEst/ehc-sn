...
class MazeHardControllerRuntime:
    ...
    MazeHard reinforcement-learning environment so the controller remains
    generic over batch-key semantics.
    """
    ...
    def build_env_step_td(  # -------------------------------------------------
        ...
        *,
        ...
        """Return the environment step TensorDict populated from controller outputs."""
        _, labels = _validate_maze_hard_batch(data)
        task_logits: Tensor = task_output.task_logits  # type: ignore[union-attr]
        step_td = env_td.clone()
        step_td["action"] = action.unsqueeze(-1) if action.ndim == 1 else action
        step_td["logits"] = task_logits.detach().to(torch.float32)
        step_td["labels"] = labels
        return step_td
