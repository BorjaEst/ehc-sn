...
class DungeonFixedProbeProvider:
    ...
    def __init__(  # ----------------------------------------------------------
        ...
        *,
        ...
        if not sample_ids:
            raise ValueError(
                "DungeonFixedProbeProvider: sample_ids must not be empty",
            )
        if len(sample_ids) != len(set(sample_ids)):
            duplicates = [s for s in sample_ids if sample_ids.count(s) > 1]
            raise ValueError(
                "DungeonFixedProbeProvider: sample_ids contains duplicates: "
    ...
    def provide_cases(  # -----------------------------------------------------
        ...
        if missing:
            raise KeyError(
                "DungeonFixedProbeProvider: sample_ids not found in split "
