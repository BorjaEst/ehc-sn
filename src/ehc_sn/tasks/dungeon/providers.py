...
Ownership rules:
    ...
    - They know dungeon channel semantics and index conventions.
...
class DungeonReplayDiagnosticProvider:
    ...
    - ``batch_size`` (*int*, default ``1``): Dungeon episodes per evaluation case batch.
    ...
    def provide_cases(self, max_batches: int = 0) -> Iterator[EvaluationCaseBatch]:
        """Yield batched dungeon replay cases from the configured split.
...
class DungeonFixedProbeProvider:
    ...
    def __init__(
        ...
        *,
        ...
        if not sample_ids:
            raise ValueError("DungeonFixedProbeProvider: sample_ids must not be empty")
        if len(sample_ids) != len(set(sample_ids)):
            duplicates = [s for s in sample_ids if sample_ids.count(s) > 1]
            raise ValueError(f"DungeonFixedProbeProvider: sample_ids contains duplicates: {sorted(set(duplicates))}")
        self._dataset_path = Path(dataset_path)
        self._split = split
    ...
    def provide_cases(self, max_batches: int = 0) -> Iterator[EvaluationCaseBatch]:
        ...
        if missing:
            raise KeyError(f"DungeonFixedProbeProvider: sample_ids not found in split {self._split!r}: {missing}")
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if self._batch_size == 1:
                ...
                case_id = f"dungeon-probe-{batch_idx:04d}"
