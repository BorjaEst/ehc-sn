"""Single ``eval`` command and shared ``inspect`` subcommand.

This module exports ``evaluation_app`` — a Typer app registered as
``eval`` in the top-level ``ehp`` CLI.

Usage::

    ehp eval arena-tem-v1 --model ./checkpoints/best.ckpt
    ehp eval run --config examples/evaluation/invocation.toml
    ehp eval inspect artifacts/eval/arena-tem-v1-20260627/
"""
...
from ehc_sn.eval.artifacts import _MANIFEST_FILENAME
...
from ehc_sn.eval.model_ref import ModelRef
from ehc_sn.eval.offline import run_offline_eval
from ehc_sn.eval.recipes import (
    EvaluationAlias,
    list_recipes,
    resolve_recipe,
)
from ehc_sn.eval.tracking import MLflowEvaluationRecorder
...
def _reject_recipe_toml(cfg: dict[str, object], path: Path) -> None:
    ...
    typer.echo(
        ...
        "  ehp-sn eval run <alias> --model <checkpoint>\n\n"
        "Example:\n"
        "  ehp-sn eval run arena-tem-v1 "
...
def _run_evaluation(
    ...
    output_dir = Path(f"artifacts/eval/{loaded.resolved.alias.value}")
    ...
    with recorder:
        ...
        eval_identity = EvaluationIdentity(
            evaluation_id=(
                str(recorder.run_id)
                if recorder.run_id
                else f"eval-{loaded.resolved.alias.value}"
            ),
...
def run(
    ...
    Two invocation forms are supported:
        ...
        ehp-sn eval run RECIPE --model MODEL [OPTIONS]
        ehp-sn eval run --config INVOCATION.toml [OPTIONS]
...
def inspect(
    ...
    if gallery and result.gallery is not None:
        ...
        if result.gallery.diagnostics:
            ...
            from ehc_sn.eval.inspection import FigureRejectionCode
