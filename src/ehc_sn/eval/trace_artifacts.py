...
from ehc_sn.eval.artifacts import _rehydrate_trace_tree
...
class ZarrEvaluationTraceReader:
    ...
    def __init__(self, zarr_path: Path, index_path: Path) -> None:
        ...
        if raw.get("schema") != "ehc_sn.eval.trace_index.v1":
            raise ValueError(
                f"Unsupported trace index schema: {raw.get('schema')!r}. "
                f"Expected 'ehc_sn.eval.trace_index.v1'."
            )
