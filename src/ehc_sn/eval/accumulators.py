...
class SpatialPopulationAccumulator(EvaluationConsumer):
    ...
    def __init__(  # ----------------------------------------------------------
        ...
        *,
        ...
        if moments not in (1, 2):
            raise ValueError(
                f"moments must be 1 or 2, got {moments}."
            )
        if n_locations < 1:
            raise ValueError(
                f"n_locations must be >= 1, got {n_locations}."
            )
    ...
    def update(self, ctx: StepContext) -> None:
        ...
        location_id: Tensor | None = (
            ctx.record.snapshot.data.get("location_id")
        )
    ...
    # ── MergeableAccumulator / distributed eval hooks ───────────────────────
    ...
    def merge(self, other: SpatialPopulationAccumulator) -> None:
        ...
        self._occupancy += other._occupancy.to(
            device=self._occupancy.device
        )
        ...
        if self._activation_sq_sum is not None:
            for band in self._activation_sq_sum:
                self._activation_sq_sum[band] += (
                    other._activation_sq_sum[band].to(
                        device=self._activation_sq_sum[band].device
                    )
                )
    ...
    def _copy_from(self, other: SpatialPopulationAccumulator) -> None:
        ...
        if other._activation_sq_sum is not None:
            self._activation_sq_sum = {
                k: v.clone()
                for k, v in other._activation_sq_sum.items()
            }
        self._valid_step_count = other._valid_step_count
...
def _assert_merge_compatible(
    ...
    if a._n_locations != b._n_locations:
        mismatches.append(
            f"n_locations: {a._n_locations} != {b._n_locations}"
        )
