...
class EHCObjective(VariationalLossHeadBase[EHCObjectiveConfig]):
    ...
    def _build_metric_ratios(
        ...
        **_: Any,
        ...
        return {
            **acc_metrics,
            EHC_LOSS_OBS_NLL_REVISIT: RatioStat(losses.loss_obs_nll_sum.detach(), protocol_count),
            EHC_LOSS_GRID_KL_REVISIT: RatioStat(losses.loss_grid_kl_sum.detach(), protocol_count),
            EHC_LOSS_PLACE_CONSISTENCY_REVISIT: RatioStat(losses.loss_place_consistency_sum.detach(), protocol_count),
            EHC_LOSS_REG_REVISIT: RatioStat(losses.loss_reg_sum.detach(), protocol_count),
            EHC_LOSS_OBS_NLL_ALL: RatioStat(all_loss_sums["loss_obs_nll_sum"], batch_count),
            EHC_LOSS_GRID_KL_ALL: RatioStat(all_loss_sums["loss_grid_kl_sum"], batch_count),
            EHC_LOSS_PLACE_CONSISTENCY_ALL: RatioStat(all_loss_sums["loss_place_consistency_sum"], batch_count),
            EHC_LOSS_REG_ALL: RatioStat(all_loss_sums["loss_reg_sum"], batch_count),
        }
    ...
    def _build_step_output(
        ...
        return EHCObjectiveStep(losses=losses, metrics=metrics, outputs=outputs, signals=signals)
    ...
    def compute_signals(
        ...
        **_: Any,
        ...
        place_transition = sum_latent_terms(mse_consistency, place_transition_relation.lhs, place_transition_relation.rhs).sum().detach()
        if place_sensory_relation is not None:
            place_sensory = sum_latent_terms(mse_consistency, place_sensory_relation.lhs, place_sensory_relation.rhs).sum().detach()
        else:
