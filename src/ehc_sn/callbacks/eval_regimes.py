...
class EvaluationRegimesCallback(pl.Callback):
    ...
    def __init__(self, settings: EvaluationRegimesCallbackSettings,) -> None:  # fmt: skip  # ------------------------------------------------------------------------------
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: LightningModule,) -> None:  # fmt: skip  # ---------------------------------------------------------------
        ...
        if not isinstance(pl_module, SupportsEvaluationRegimes):
            log.warning(
                "EvaluationRegimesCallback: pl_module '%s' does not implement SupportsEvaluationRegimes. " "All regimes will be skipped.",
    ...
    def _should_run(self, trainer: Trainer, regime: EvaluationRegimeSettings,) -> bool:  # fmt: skip  # ---------------------------------------------------------------------------
        ...
        """
        epoch_ok = regime.schedule.every_n_epochs > 0 and (trainer.current_epoch % regime.schedule.every_n_epochs == 0)
        step_ok = regime.schedule.every_n_steps > 0 and (trainer.global_step % regime.schedule.every_n_steps == 0)
        # If both are disabled, fall back to every epoch.
        if regime.schedule.every_n_epochs == 0 and regime.schedule.every_n_steps == 0:
            return True
        return epoch_ok or step_ok

    def _run_regime(self, trainer: Trainer, pl_module: SupportsEvaluationRegimes, regime: EvaluationRegimeSettings,) -> None:  # fmt: skip  # ---------------------------------------------------------------------------
        ...
        if result.n_batches == 0:
            log.warning(
                "EvaluationRegimesCallback: regime '%s' provider '%s' yielded 0 batches. " "No metrics logged.",
                regime.regime_id,
