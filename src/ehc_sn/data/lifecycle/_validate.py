...
def _validate_structure(root: Path) -> dict[str, Any]:
    ...
    if dataset_class == "task_corpus":
        # parents must be a non-empty dict with role-addressed refs
...
def _validate_path_grammar(root: Path, manifest: dict[str, Any]) -> None:
    ...
    if dataset_class == "shared_substrate":
        ...
        # parents dict with role-addressed refs.
        parents: dict = manifest.get("parents", {})
        if not parents:
            raise ValueError(
                f"Task corpus manifest_schema_version must have a non-empty 'parents' dict."
            )
        for role, ref in parents.items():
            pfam = ref.get("family")
            proot = ref.get("root", "")
