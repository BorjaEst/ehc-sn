"""Generic rollout controller for variational models.
...
class VAROutput(DetachMixin):
    """Semantic output contract for aggregate variational heads.
    
    Latent codes can be a single tensor or a sequence of tensors, depending on the model's design.
    """

    obs_logits: Tensor  # (B, S, V) observation likelihood logits
    latent_post: LatentCode  # Latent code(s) produced by the backbone
    latent_prior: LatentCode  # Latent code(s) produced by the backbone's prior head
...
class VARController[BkState](BaseController[BkState, VARControllerConfig]):
    """Rollout controller for generic variational objectives.
    ...
    The controller:
        ...
        - Expects the backbone to produce observation logits and latent codes for both posterior and prior.
            
    This controller is suitable for training models where the loss is composed of an observation
    likelihood term and a latent consistency term. 
    """
