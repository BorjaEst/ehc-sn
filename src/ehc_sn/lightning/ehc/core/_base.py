...
def resolve_spatial_core_ids(  # ----------------------------------------------
    ...
    return {id(p) for name in _SPATIAL_CORE_NAMES for p in getattr(model, name).parameters()}
...
def resolve_controller_bridge_ids(  # -----------------------------------------
    ...
    return {id(p) for name in _CONTROLLER_BRIDGE_NAMES for p in getattr(model, name).parameters()}
...
def resolve_controller_heads_ids(  # ------------------------------------------
    ...
    return {id(p) for p in model.pfc.estimator.parameters()} | {id(p) for p in model.str.parameters()}
...
def load_weights_from_checkpoint(  # ------------------------------------------
    ...
    if unknown:
        raise ValueError(f"Unknown semantic groups: {unknown!r}. " f"Valid groups: {sorted(_GROUP_TO_PREFIXES)}.")
    ...
    stripped: dict = {(k[len("model.") :] if k.startswith("model.") else k): v for k, v in raw_sd.items()}

    filtered = {k: v for k, v in stripped.items() if any(k == p or k.startswith(f"{p}.") for p in prefixes)}
    if not filtered:
        raise ValueError(
