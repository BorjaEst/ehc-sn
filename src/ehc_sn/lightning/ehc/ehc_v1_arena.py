...
# =================================================================================================
class ModelConfig_EHC_V1(BaseModel, extra="forbid"):
    """Top-level EHC v1 training config."""

    # ~~ Model architecture ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    model_config_path: Path = Field(
        ...,
        description="Path to the model configuration TOML file that specifies the EHC v1 architecture.",
    )
    adapter: ArenaEHCAdapterSettings = Field(
        ...,
        description="Settings for the arena bridge adapter that binds EHC v1 to task inputs/outputs.",
    )
    ...
    objective: EHCObjectiveConfig = Field(
        ...,
        description="EHC objective configuration.",
    )
    ...
    arena_phase: Literal["arena_spatial_pretrain"] = Field(
        ...
        description="Arena training phase. Controls which model parameters the optimizer updates.",
    )
    ...
    def validate_adapter_contract(self) -> "ModelConfig_EHC_V1":
...
class TrainingModel(L.LightningModule):
    ...
    @property
    def config(self) -> ModelConfig_EHC_V1:
    ...
    def _require_train_controller(self) -> ReplayTrajectoryController:
        ...
        if self.train_controller is None:
            raise RuntimeError("EHC training runtime is not initialized.")
    ...
    def _require_eval_controller(self) -> ReplayTrajectoryController:
        ...
        if self.eval_controller is None:
            raise RuntimeError("EHC evaluation runtime is not initialized.")
    ...
    def configure_optimizers(  # -------------------------------------------------------------------
        ...
        # arena_spatial_pretrain: update only LEC, MEC, HPC, lec_to_hpc, mec_to_hpc, _decoder.
        # Forbidden: model.pfc (incl. pfc.estimator), model.str, model.pfc_to_hpc, model.hpc_to_pfc.
        _excluded_ids = (
            {id(p) for p in self.model.pfc.parameters()}
            | {id(p) for p in self.model.str.parameters()}
            | {id(p) for p in self.model.pfc_to_hpc.parameters()}
            | {id(p) for p in self.model.hpc_to_pfc.parameters()}
        )
        sup_params = [p for p in self.bridge_adapter.parameters() if p.requires_grad and id(p) not in _excluded_ids]
    ...
    ) -> EHCRuntimeState:  # fmt: skip
        """Resolve and apply EHC runtime dynamics for the current global step."""
        runtime = resolve_ehc_runtime(step, self.config.runtime)
    ...
    def _reset_train_stream(self) -> None:
        ...
        EHC carry includes parameter-dependent associative memory. Reusing that
    ...
    def training_step(  # -------------------------------------------------------------------------
        ...
        """Run one EHC chunked-TBPTT optimizer update through the recurrent runner."""
    ...
    def validation_step(  # -----------------------------------------------------------------------
        ...
        """Run a full EHC rollout through the recurrent runner and trace observer."""
        ...
        update_metric_collection_from_evaluated_chunk(self.val_metrics, evaluation.evaluated, EHC_EPISODE_ROUTES)
