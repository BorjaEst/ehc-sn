...
class EvaluationTraceRequest(BaseModel, extra="forbid"):
    ...
    enabled: bool = Field(
        default=False,
        description="Activate trace collection for this regime.",
    )
    trace_keys: list[str] = Field(
        default_factory=list,
        description="Semantic trace key names required.",
    )
...
class EvaluationScheduleSettings(BaseModel, extra="forbid"):
    ...
    every_n_epochs: int = Field(
        default=0,
        ge=0,
        description="Run every N epochs (0 = disabled).",
    )
    every_n_steps: int = Field(
        default=0,
        ge=0,
        description="Run every N global steps (0 = disabled).",
    )
    max_batches: int = Field(
        default=1,
        ge=0,
        description="Provider case batches to consume per run (0 = unlimited).",
    )
...
class EvaluationRegimeSettings(BaseModel, extra="forbid"):
    ...
    regime_id: str = Field(
        ...,
        description="Unique regime identifier (used as metric namespace suffix).",
    )
    phase_kind: Literal["diag", "bench"] = Field(
        default="diag",
        description="Phase kind: 'diag' or 'bench' (future).",
    )
    provider_ref: str = Field(
        ...,
        description="Dotted import path to a provider factory or class.",
    )
    provider_settings: dict[str, Any] = Field(
        default_factory=dict,
        description="Forwarded to the provider factory.",
    )
    schedule: EvaluationScheduleSettings = Field(
        default_factory=EvaluationScheduleSettings,
        description="Scheduling settings.",
    )
    trace_request: EvaluationTraceRequest = Field(
        default_factory=EvaluationTraceRequest,
        description="Trace collection settings.",
    )
