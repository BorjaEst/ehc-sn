...
def load_provider(  # ---------------------------------------------------------
    ...
    if not module_path:
        raise ImportError(
            f"provider_ref '{regime.provider_ref}' must be a dotted path (module.ClassName)."
        )
...
def run_evaluation_regime(  # -------------------------------------------------
    *,
    ...
    metric_collection: MetricCollection = pl_module.build_evaluation_metrics(
        namespace
    )
    ...
    def _transfer_batch(batch: Any) -> Any:
        ...
        try:
            ...
            log.debug(
                "Regime runner: batch transfer failed; using batch as-is.",
                exc_info=True,
            )
        return batch
    ...
    if trainer is not None:
        try:
            ...
            log.debug(
                "Regime runner: metric collection device transfer failed; leaving on default device.",
                exc_info=True,
            )

    trace_request = (
        regime.trace_request if regime.trace_request.enabled else None
    )
    latest_artifact: Optional[EvaluationBatchArtifacts] = None
    n_batches = 0

    for case_batch in provider.provide_cases(
        max_batches=regime.schedule.max_batches
    ):
        batch = _transfer_batch(case_batch.batch)
        artifact = pl_module.execute_evaluation_batch(
            batch, trace_request, case_batch.source_context
        )
        ...
        if artifact.apply_to_metrics is not None:
            try:
                ...
                log.debug(
                    "Regime runner: metric update raised an error for regime '%s'.",
                    regime.regime_id,
                    exc_info=True,
                )
    ...
    if n_batches > 0:
        try:
            ...
            log.debug(
                "Regime runner: metric compute raised an error for regime '%s'.",
                regime.regime_id,
                exc_info=True,
            )
