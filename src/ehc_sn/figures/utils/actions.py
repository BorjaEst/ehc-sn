...
def action_patch(location_from: dict, location_to: dict, radius: float, colour) -> plt.Polygon:
    ...
    if location_to["id"] == location_from["id"]:
        # Self-transition: arrow points down in image/grid coordinates.
        a_dir = np.pi / 2
        xdat = location_from["o"] + radius * np.array(
            [2 * np.cos(a_dir - np.pi / 6), 2 * np.cos(a_dir + np.pi / 6), 3 * np.cos(a_dir)]
        )
        ydat = (
            location_from["y"]
            - radius * 3
            + radius
            * np.array([2 * np.sin(a_dir - np.pi / 6), 2 * np.sin(a_dir + np.pi / 6), 3 * np.sin(a_dir)])
        )
    else:
        xvec = location_to["o"] - location_from["o"]
        yvec = location_to["y"] - location_from["y"]
        ...
        ydat = location_from["y"] + radius * np.array(
            [2 * np.sin(a_dir - np.pi / 6), 2 * np.sin(a_dir + np.pi / 6), 3 * np.sin(a_dir)]
        )
