...
# =============================================================================
class ArenaTEMV1AdapterSettings(BaseModel, extra="forbid"):
    """Task-side Arena settings required to bind the TEM v1 core."""
...
class ArenaTEMV1Diagnostics(DetachMixin):
...
class ArenaInputsEncoderV1(nn.Module):
    ...
    The arena step payload carries ``observation_id`` (integer) but not an
    encoded ``observation`` vector.  This encoder materialises the one-hot
    observation from the observation id, then applies the TwoHot lookup.
    """
    ...
    def __init__(
        ...
        if encoder_config.kind != "two_hot":
            raise ValueError(
                f"ArenaInputsEncoderV1: encoder kind {encoder_config.kind!r} is not yet "
                "implemented. Only 'two_hot' is supported in this release."
            )
        if encoder_config.layout != "replicated":
            raise ValueError(
                f"ArenaInputsEncoderV1: layout {encoder_config.layout!r} is not supported " "with kind='two_hot'. Use layout='replicated'."
            )
        self.encoder = TwoHotEncoder(observation_dim, feature_dim)
        self._obs_dim = observation_dim
        self._n_freq = n_freq

    def forward(self, batch: dict[str, Tensor]) -> TEMInputV1:
...
class ArenaOutputsDecoderV1(nn.Module):
    """Decodes a :class:`TEMOutputV1` into an :class:`ArenaTEMV1BridgeOutput`."""
    ...
    def forward(self, model_output: TEMOutputV1) -> ArenaTEMV1BridgeOutput:
        ...
        tem = ArenaTEMV1Diagnostics(obs_logits=ol, grid_codes=gc, place_codes=pc)
        return ArenaTEMV1BridgeOutput(task=task, tem=tem)


# =============================================================================
class ArenaTEMV1BridgeAdapter(nn.Module):
    """Arena plus TEM v1 bridge adapter.

    Implements the :class:`~ehc_sn.controllers._base.RolloutBackbone` protocol.
    :meth:`prepare_inputs` encodes an arena step payload into a model-native
    :class:`TEMInputV1`; :meth:`prepare_outputs` decodes a :class:`TEMOutputV1`
    into the split task + TEM surfaces.
    """

    def __init__(self, model: TEMModelV1, config: ArenaTEMV1AdapterSettings) -> None:
        """Initialize the arena plus TEM v1 bridge adapter."""
        super().__init__()
        self._config = config
        self.model = model
        self.encoder = _build_encoder_v1(model, config)
        self.decoder = _build_decoder_v1(model, config)

    @property
    def config(self) -> ArenaTEMV1AdapterSettings:
        """The arena plus TEM v1 bridge adapter settings."""
        return self._config
    ...
    def prepare_inputs(self, batch: Batch) -> TEMInputV1:
        """Encode a pre-extracted arena step dict into a model-native :class:`TEMInputV1`."""
        return self.encoder(batch)

    def prepare_outputs(self, model_output: TEMOutputV1) -> ArenaTEMV1BridgeOutput:
    ...
    ) -> tuple[ArenaTEMV1BridgeOutput, TEMStateV1]:
        """Run a forward pass of the TEM v1 bridge adapter on an arena step payload."""
...
# =============================================================================
def _build_encoder_v1(model: TEMModelV1, config: ArenaTEMV1AdapterSettings) -> ArenaInputsEncoderV1:
...
def _build_decoder_v1(model: TEMModelV1, config: ArenaTEMV1AdapterSettings) -> ArenaOutputsDecoderV1:
...
__all__ = [
    ...
    "ArenaTEMV1Diagnostics",
    "ArenaTEMV1AdapterSettings",
    "ArenaTEMV1BridgeAdapter",
    "ArenaTEMV1BridgeOutput",
]
