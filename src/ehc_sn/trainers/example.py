# Copyright (c) 2025, lightning Fabric example trainer
...
class MyCustomTrainer:
    ...
    # Optimizer Parsing Inconsistency & Inconsistent Error Handling
    def fit(
        self,
        ...
        Raises:
            NotImplementedError: If FSDP strategy is used, which is currently not supported.
            ValueError: If no valid optimizer could be parsed from configure_optimizers.
        ...
        if optimizer is None:
            raise ValueError("No valid optimizer found. Check your `configure_optimizers`.")
    ...
    # Dictionary Update Return Value Issues
    def _parse_optimizers_schedulers(self, configure_optim_output) -> tuple[
        ...
        if isinstance(configure_optim_output, (list, tuple)):
            if all(isinstance(_opt_cand, L.fabric.utilities.types.Optimizable) for _opt_cand in configure_optim_output):
                # single optimizer in list - FIX: incorrect indexing in optimizer parsing
