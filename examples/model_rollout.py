...
CONFIGURATION_PATH = os.environ.get(
    "EXP01_CONFIGURATION_PATH", "config/defaults_ehc.toml"
)
...
class ExampleArguments(BaseSettings, extra="forbid", cli_parse_args=True):
    ...
    architecture: PFCSettings = Field(
        ...,
        description=(
            "Architecture config for the HRM model. The keys in `architecture` "
            "are passed to the HRModel constructor."
        ),
    )
    ...
    @property
    def model(self) -> HRMV1ModelConfig:
        """Construct a HRMV1ModelConfig from the provided arguments."""
    ...
    dataset_path: Path = Field(
        ...,
        description=(
            "Path to the versioned dataset root (contains index.jsonl and "
            "per-split per-channel NPY arrays).",
        ),
    )
    seed: int = Field(
        42,
    ...
    augment: bool = Field(
        False,
        description=(
            "Apply RandomDihedral augmentation (disabled by default for eval "
            "rollouts)."
        ),
    )
...
def main() -> None:
    ...
    act_options = {
        "allow_halt": False,
        "explore": False,
    }  # Disable halting and exploration during eval
    carry0 = model.step_module.initial_carry(batch)

    with torch.no_grad():
        for t, step in StepLoop(
            model.step_module, step_batches, carry0, options=act_options
        ):
