...
environments, models, and the EHC rollout controller.
...
| `MazeHardEnv`                   | `input_ids`, `step_count`                                                                                         | none                                                            | Environment TensorDict                                          | next TensorDict                                                                                      | Mechanical env scaffold: static token tape, step count, halt/truncation only. Not the active HRM v2 deliberation surface; `labels` and `prev_accuracy` are task-owned, not env keys. |
